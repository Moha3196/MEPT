...
if(isset($_POST["submit"])) {
	if ($_FILES["uploadedClasses"]["error"] == 0) {  //if we get an error, when attempting to upload a file
		...
		$temp_arr_classes = explode(PHP_EOL,$classes_as_string);  //creates an array of strings, where each string in the array is just one line from the CSV-file (PHP_EOL means "PHP End Of Line", i.e. "where this line ends")$temp_arr_students = explode(PHP_EOL,$students_as_string);
	...
	if ($_FILES["uploadedStudents"]["error"] == 0) {  //if we get an error, when attempting to upload a file
		...
		$counter = 0;  //a counter for keeping track of index - will make more sense later  //creates an array of strings, where each string in the array is just one line from the CSV-file (PHP_EOL means "PHP End Of Line", i.e. "where this line ends")
		$temp_arr_students = explode(PHP_EOL,$students_as_string);$line_arr_classes = array(count($temp_arr_classes));  //an array of all the lines (that will eventually exist, just wait) in the uploaded CSV-file
		$line_arr_students = array(count($temp_arr_students));
		//var_dump($temp_arr_classes);
		//echo "<br>";
		
		foreach($temp_arr_students as $entry)  //each of the newly created strings is saved under the variable name "entry" since it will be used as an entry in the database later on
		{
			if($counter != 0)
			{
				$line = explode(",", $entry); //now we split each entry where a comma is present
				$line_arr_students[$counter-1] = $line; //saves all the lines just made to an array. Uses counter as an index to cycle through the entire array
				//var_dump($line_arr_classes[$counter-1]);
			}
			$counter++; //adds 1 to the counter, so that all the different indexes are used, and not just one of them
		}
		...
		*/
		...
		{
			$mysql_internal = Connect();  //connects to database
			$sql_req = "INSERT INTO users (name, username, password, type, inClasses) VALUES (" . "'" . $row[0] . "','" . $row[1] . "','" . hash("sha512", $row[2]) . "', 0, '" . $_POST["ClassName"] . "')";  //stores the values from the new "row" array in the database
			$mysql_internal->query($sql_req); //sends this to the database
			$mysql_internal->close(); //closes the database, since we're done using it
			
			$counter++;
		}
		//readfile($_FILES["uploadedClasses"]["tmp_name"]);    |    " . "'" . $row[0] . "','" . $row[1] . "','" . hash("sha512", $row[2]) . "', 0,"' . $_POST["ClassName"] . '"
	}
	else {
		echo 'Upload failed. Error is: ' . $_FILES["uploadedClasses"]["error"];  //prints the error, if we get one
		$exitStatus = 0;  //changes the status to 0
